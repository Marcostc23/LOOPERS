        ...
        <?php while ($vinilo = $result_vinilos->fetch_assoc()): ?>
            <div class="col-sm-6 col-lg-3 mb-4">
                <div class="card-vinilo">
                    <?php 
                        // Corregimos la ruta: Si no empieza por '../', se lo añadimos
                        $img = $vinilo['foto'];
                        if (!empty($img) && strpos($img, '../') === false) {
                            $img = '../frontend/' . $img;
                        }
                        $rutaFinal = (!empty($img) && file_exists($img)) ? $img : '../frontend/imgs/default_album.png';
                    ?>
                    <img src="<?= $rutaFinal ?>?v=<?= time() ?>" alt="Portada">
                    <h5 class="text-warning"><?= htmlspecialchars($vinilo['nombre']) ?></h5>
                    <p class="small text-secondary"><?= htmlspecialchars($vinilo['autor']) ?></p>
